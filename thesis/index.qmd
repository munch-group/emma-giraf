...
$$
T_k \sim \mathrm{Exp}\left(\binom{k}{2}\right)
$$
...
Hvor hver $T_k$ er eksponentialfordelit med parameter $\binom{2}{k}$

Genealogiske træer genereret af coalescent modellen danner grundlaget for mange statistikker i populationsgenetik. Mutationer antages typisk at opstå langs grenene i det genealogiske træ som en Poisson-proces med rate $\theta/2$. Antallet af mutationer på en gren med længde $t$ følger dermed en Poissonfordeling

$$
M \sim \mathrm{Poisson}\left(\frac{\theta}{2} t\right)
$$
...
$$
f(t) = \boldsymbol{\alpha} e^{St} \boldsymbol{s}
$$

Hvor
$$
\boldsymbol{s} = -S \mathbf{1}
$$
