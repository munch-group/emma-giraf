...
populationsstruktur eller balancerende selektion {[}Kilde{]}. Mens
...
estimere populationsgenetiske parametre for seks \emph{Papio}-arter.
...
Sojourn-tiderne for \(n=4\) viser, at processen i gennemsnit tilbringer
\(0,167\) coalescent-enheder med fire aktive linjer, \(0,333\) med tre
aktive linjer og \(1,000\) med to aktive linjer. Den sidste fase, hvor
der kun er to linjer tilbage, udgør dermed den største del af den
forventede tid til MRCA (Table~\ref{tbl-sojourn}).
...
den samlede grenlængde er \(\mathbb{E}[L] = 3{,}667\). Dette stemmer
...
Viser desuden af grenlængderne i de forskellige frekvensklasser ikke er
uafhængige. Singleton- og doubleton-grenlængderne er negativt
korrelerede med \(\text{Cov}(B_1, B_2) = -0{,}222\), mens singleton og
tripleton er positivt korrelerede med
\(\text{Cov}(B_1, B_3) = 0{,}889\). Dette viser, at en antagelse om
uafhængighed mellem frekvensklasserne vil undervudere usikkerhenden i
...
Efter verificeringen af den analytiske model blev SVGD-inferensen
kalibreret på simulerede data med kendte parametre. Først blev
forskellige priorvalg sammenlignet ved en sand værdi på \(\theta\) og
\(n=2000\) observation. Prior-sensitiviteten blev derefter undersøgt
over flere bavian-realistiske \(\theta\)-værdier. Resultaterne i
Table~\ref{tbl-prior-bav} viser, at DataPrior generelt gav lav bias og
lav posterior-standardafvigelse på tværs af de testede værdier. Brede
Gaussiske priors var derimod ustabile, især ved høje \(\theta\)-værdier,
hvor estimaterne kunne blive stærkt biasede. HalfCauchy-prioren havde
større spredning, men er relevant i modeller, hvor migrationsrater skal
kunne antage både lave og høje værdier. Table~\ref{tbl-prior-bav} viser,
hvordan de seks priors klarer sig over bavian-realistiske
\(\theta\)-værdier. DataPrior giver den laveste bias og std på tværs af
alle testede værdier. Det er ikke overraskende, da DataPrior bruger
Wattersons estimator til automatisk at centrere prioren tæt på data,
inden SVGD startes. Den brede GaussPrior og HalfCauchy er til gengæld
ustabile ved høje \(\theta\)-værdier som \(\theta = 20\), med std op til
\(95\) og bias op til \(60\) --- det er klart ubrugeligt. DataPrior
...
\(\theta_0 = 0{,}7\) og \(\theta_1 = 1{,}0\). Venstre figur viser
...
den relative afvigelse i middelværdi er \(3{,}18\%\) hvilket er inden
for \(5\%\), så modellen er korrekt implementeret. Højre figur viser
CDF-sammenligningen, hvor de to kurver næsten ikke kan skelnes.
...
Figure~\ref{fig-msprime-svgd-twoisland}. Begge sande parametre
(\(\theta_0 = 0{,}7\) og \(\theta_1 = 1{,}0\)) genfindes inden for
posteriorintervallet. Konvergensplottet viser en tydelig forskel:
\(\theta_0\) (blå) stabiliserer sig hurtigt med et smalt interval, mens
\(\theta_1\) (orange) svinger mere og har bredere usikkerhed.
...
I forhold til migration, testet der hvad der sker vil forksellige
scenarier af migration (Table~\ref{tbl-identifiability}). Med fast
\(\theta_0 = 0{,}7\) og \(5000\) observationer estimeres \(\theta_0\)
stabilt i alle scenarier med std \(0{,}013\)--\(0{,}057\).
Migrationsraten \(\theta_1\) er derimod meget afhængig af scenariet: std
er \(0{,}048\) ved lav migration, stiger til \(0{,}163\) ved moderat og
ender på \(2{,}139\) ved høj migration. En std der er større end
estimatets middelværdi gør at estimatet er ubrugeligt.
...
(\(\theta_1 = 0{,}2\)). Her ses det, at partiklerne er tæt samlet
omkring de sande værdier og \(\theta_1 = 0{,}2\) genfindes. Lav
migration giver en TMRCA-forlængelse der er let at genkende i data.
...
TMRCA er simpelthen ikke følsom over for \(\theta_1\) i dette interval.
...
(\(\theta_1 = 5{,}0\)). Partiklerne er nu spredt langs en lang
diagonalen. Det afspejler, at mange kombinationer af \(\theta_0\) og
\(\theta_1\) giver næsten identisk TMRCA-fordeling. Dermed er høj
migration og lav coalescentrate svær at skelne fra lav migration og høj
coalescentrate.
...
En sammenligning af SVGD med MoM over fem bavian-realistiske scenarier
med \(n=3000\) vises i Table~\ref{tbl-mom-bavian}. I scenariet
\([0{,}5;\; 5{,}0]\) estimerer MoM \({\theta}_1 = 2{,}39\) mod sand
\(0{,}5\). I de andre sccenarier er MoM dog bedre. Generelt ses der at
SVGD giver bedre estimater i \(4\) af \(5\) scenarier og giver desuden
usikkerhedskvantificering, som MoM ikke har.
...
flaskehals-dybde \(f = N/N_\text{bottle}\). I venstre figur ses at en
...
moderat flaskehals (\(f = 2\)) ændrer næsten ingenting.
\(\mathbb{E}[\text{TMRCA}]\) forbliver \(1{,}500\) coalescentenheder
helt op til \(f = 10\) falder kun til \(1{,}003\) ved \(f = 50\). Dermed
sætter moderate flaskehalse altså ikke særligt store spor i den
forventede coalescenttid.
...
(ancestral). Dermed er den moderne epoke sværere at estimere fordi den
kun dækker en kortere tidsperiode. msprime-valideringen giver
\(0{,}13\%\) afvigelse, som bekræfter at modellen er korrekt
implementerede.
...
\texttt{reward\_limit}, der bestemmer, hvor mange mutationer per locus
...
mutationsrater. I venstre figur ses at \(\mu = 0{,}5\) (blå) falder
hurtigt under \(5\%\)-grænsen (rød stiplet) allerede ved
\texttt{reward\_limit\ =\ 2}, mens \(\mu = 1{,}0\) (orange) kræver
\texttt{reward\_limit\ =\ 3} for at nå deficit \(0{,}21\%\). Højre figur
viser, at tabelstørrelsen vokser kubisk med \texttt{reward\_limit},
hvilket gør at standardkonfigurationen er \(\mu = 1{,}0\) og
...
estimeres \(\hat{\theta} = 0{,}663\) hvilket er et underskud på en
faktor \(\sim 10\). Med forkert \(\mu = 2{,}5\) estimeres
\(\hat{\theta} = 36{,}4\) hvilket giver et overskud på en faktor
\(\sim 5\). Biasset er systematisk, forsvinder ikke med mere data og er
lineært. Det understreger at en korrekt \(\mu\) er afgørende.
...
\(9{,}4\%\) ved \(\theta = 2\), \(27{,}3\%\) ved \(\theta = 20\). Ved
\(\theta = 5\) ses en gevinst på \(17,2 \%\) og \(theta=10\) en gevinst
på \(65,1\%\). Multi-feature SFS er altså mest fordelagtigt ved moderate
\(\theta\)-værdier.
...
Figure~\ref{fig-arg-pairwise} viser to-locus ARG-analysen. Der ses lav
korrelation mellem \(\hat{\theta}_0\) og rekombinationsraten
\(\hat{R}\), som indikerer at de to parametre sperat er identificerbar.
\(\hat{\theta}_0 = 3{,}470 \pm 0{,}069\) (sand \(3{,}0\)) genfindes
præcist. \(\hat{R} = 0{,}654 \pm 0{,}082\) (sand \(0{,}5\)) har lidt
opadgående bias, fordi rekombination er svær at skelne fra
coalescent-variation ved moderate \(\rho\).
...
\(D\) for alle seks arter. De to figurer giver information om \(\pi\)
siger noget om det absolutte niveau af genetisk variation, mens Tajimas
\(D\) siger noget om SFS-formen og dermed demografisk historie. Arterne
med negativ \(D\) er \emph{P. cynocephalus} (\(D = -0{,}589\)) og
\emph{P. anubis} (\(D = -0{,}637\)), hvilket betyder at de har et
overskud af singletons. \emph{P. papio} (\(D = 1{,}019\)) har det
højeste positive \(D\) og dermed relativt færre singletons,
overensstemmelse med en historisk flaskehals.
...
\(N_e = 11.366\) (CI: \(10.252\)--\(12.480\)) altså en faktor \(\sim 9\)
forskel.
...
\emph{P. cynocephalus}. Venstre figur viser histogrammet af \(2000\)
simulerede TMRCA-tider (blå) mod phasics analytiske PDF (orange). De to
kurver følger hinanden tæt. Højre figur viser CDF-sammenligningen, som
understøtter dette. Den relative afvigelse er \(0{,}42\%\), som
...
To-ø modellen er kørt for tre artspar. Table~\ref{tbl-two-island-bavian}
viser estimaterne. Det første man lægger mærke til er, at alle tre
artspar giver \(\hat{\theta}_0 \approx 0{,}18\)--\(0{,}21\), svarende
til \(N_e\) på \(5\)--\(6\) millioner i to-ø modellen altså en faktor
\(50\)--\(70\) højere end enkeltpopulationsestimaterne fra NB07A. Det
skyldes at to-ø modellen estimerer en fælles coalescentrate for begge
populationer og den høje migration giver lange TMRCA-tider, som modellen
tolker som en meget stor ancestral population. \(N_e\) for den enkelte
art skal altså hentes fra enkeltpopulationsanalysen.
...
For migrationsraten \(\hat{\theta}_1\) ses at standardafvigelse i det
første artspar er større end selve estimatet, hvor ved de andre to
artspar er det mindre. For \emph{P. cynocephalus} × \emph{P. anubis} er
estimatet \(1{,}1378 \pm 1{,}88461\). For \emph{P. kindae} × \emph{P.
cynocephalus} er det \(0{,}6326 \pm 0{,}18379\), og for \emph{P.
hamadryas} × \emph{P. anubis} \(0{,}6913 \pm 0{,}57688\). Der ses altså,
at usikkerheden falder altså fra første til tredje artspar.
...
anubis} partiklerne spredt omkring diagonalen. For de øvrige artpar er
partiklerne mere kompakte langs diagonal.
...
coalescentenheder, mens phasics PDF er centreret omkring \(5\)--\(6\).
Den relative afvigelse er \(73{,}69\%\) hvilket er langt over
\(5\%\)-grænsen. Det bekræfter, at de estimerede parametre befinder sig
i et sted, hvor modellen ikke er identificerbar.
...
posterior for de to coalescentrater. Figure~\ref{fig-im-ci} viser
...
\includegraphics[width=8.21875in,height=3.11458in]{index_files/figure-latex/..-notebooks-07b_bavian_modeller-fig-msprime-val-im-output-1.png}
